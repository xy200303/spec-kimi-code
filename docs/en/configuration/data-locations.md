...
- **`<workspace>/specs/<name>/`**: when `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` is enabled, project-local Plan mode documents. `spec.md` records requirements, design, the task checklist, and decisions; `delivery.md` records the delivery record; this directory is outside `KIMI_CODE_HOME` so it can be committed with the project.
