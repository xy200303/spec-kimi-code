...
| `KIMI_CODE_EXPERIMENTAL_SPEC_CODING` | Enable project-local specification and design documents in Plan mode | `1`, `true`, `yes`, `on` |
| `KIMI_CODE_SPEC_QUALITY_GATE` | Select the evidence checklist for a spec coding delivery record | `fast`, `standard` (default), `strict`, `release` |
