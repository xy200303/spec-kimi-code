...
When `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` is enabled, Plan mode creates project-local documents at `<workspace>/specs/<name>/`: `spec.md` starts with frontmatter plus Goal, Acceptance Criteria, Constraints, Technology Choices, Task Checklist, Risks and Mitigation, Key Decisions, Open Questions, and Change Log sections; `delivery.md` is the delivery-record skeleton. In Plan mode, `Write` and `Edit` may only modify `spec.md`, so it can be reviewed and committed with the project. The Goal and Acceptance Criteria sections must be filled before `ExitPlanMode` presents `spec.md` for approval. After approval, the agent implements the task checklist; checking off tasks in `spec.md` is the progress update. Once implementation and verification are complete, the agent fills in `delivery.md` from its template.

**`EnterPlanMode`** accepts an optional `name` parameter: a semantic kebab-case directory name such as `"nebula-effect"`, used to create `specs/<name>/`. If omitted or invalid, a random name is used; if the directory already exists, a suffix (`-2`, `-3`, …) is appended automatically. On success it returns workflow guidance and the paths to `spec.md` and `delivery.md`.
...
When `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` is enabled, the task checklist lives directly in the "Task checklist" section of `spec.md`. The agent checks off tasks during implementation; there is no separate task-tracking tool.
