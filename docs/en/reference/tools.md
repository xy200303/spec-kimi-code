...
When `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` is enabled, Plan mode instead creates project-local documents at `<workspace>/specs/<plan-id>/`: `spec.md` starts with goal, constraints, and acceptance-criteria sections, while `design.md` starts with tasks, risks, and verification sections. `Write` and `Edit` are restricted to these two documents, so both can be reviewed and committed with the project. Fill all six sections before requesting approval; `ExitPlanMode` presents `design.md` only after this check passes. It also routes the approved documents to a development strategy (`planning`, `agile_mvp`, `controlled_feature`, `bug_diagnosis`, `refactor`, `review`, `release`, or `research`) with an explainable recommended quality gate and required task categories. The explicit `KIMI_CODE_SPEC_QUALITY_GATE` setting still controls the checklist. At approval, Kimi Code freezes the specification and design as the run's source of truth and records the approval source, time, and selected option, so later edits cannot alter its delivery record. Set that variable to `fast`, `standard` (the default), `strict`, or `release` to select the evidence checklist written into that record.

**`EnterPlanMode`** accepts no parameters; upon success it returns workflow guidance and the plan file path. With spec coding enabled, it returns the project-local specification, design, and delivery-record paths instead.
...
| `SpecRun` | Auto-allow | Read the approved run's goal, constraints, acceptance criteria, and verification approach (experimental) |
...
When `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` is enabled, **`SpecTaskList`** keeps the structured task trace for the current spec run. Each task has a stable `id`, a reason, optional strategy `category`, risk level, planned and changed file paths, and verification evidence. `SpecRun` lists the categories required by the selected strategy; assign each one to a completed task, such as `reproduction`, `root_cause`, and `regression_test` for bug diagnosis. Omitting `tasks` queries the list; providing it replaces the current task trace. Set `activeTaskId` before `Write`, `Edit`, `Bash`, `Agent`, or `AgentSwarm` calls to record those outcomes against the selected task; pass `null` to clear the selection. Only pending and in-progress tasks can be active; completing or blocking a selected task clears the selection. A low-risk active task automatically approves these supported changes after user rules, sensitive-file safeguards, and Git control-path safeguards; a medium-risk task follows the normal permission mode; a high-risk task always requests approval, including in auto mode. When `SpecDelivery` finalizes a run, the task list and trace stop accepting updates and the active task is cleared; start a new spec run for later work.

**`SpecRun`** reads the immutable snapshot and approval audit captured when the spec plan was approved. Call it during implementation to verify the goal, constraints, acceptance criteria, selected strategy, required task categories, quality gate, planned tasks, task-progress summary, current task status, planned risks, verification approach, approval source, approval time, selected option, and delivery finalization time before making further changes. It also reports whether the current `spec.md` or `design.md` has drifted from the approved snapshot; `SpecDelivery` continues to use the approved content. It accepts no parameters and is unavailable until final approval.

**`SpecDelivery`** writes both a human-readable `delivery.md` and a machine-readable `delivery.json` in the project-local run directory. Both use the immutable approved specification and design snapshot, approval audit, structured tasks, and traced tool outcomes. The Changes section maps every recorded file to its task, reason, risk, strategy category, and tool calls; the Activity section records each traced command or delegated Agent call with its outcome and task. Each quality-gate evidence item must include the `toolCallId` of a successful foreground `Bash` call that belongs to a completed spec task, so the record links evidence to a verified command; background calls and commands outside a completed task cannot prove completion. Supply any decisions, risks, open questions, and rollback notes. Set `complete` to `true` only after every task is done, every required strategy category is assigned to a completed task, and all evidence categories required by the selected quality gate have verified command references from completed tasks. Completion writes a finalization time to both records and locks them against further updates; start a new spec run for later work. When a check is missing, the tool leaves both records unchanged and explains what is missing.
