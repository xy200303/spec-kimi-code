...
When `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` is enabled, Plan mode instead creates project-local documents at `<workspace>/specs/<plan-id>/`: `spec.md` starts with goal, constraints, and acceptance-criteria sections, while `design.md` starts with tasks, risks, and verification sections. `Write` and `Edit` are restricted to these two documents, so both can be reviewed and committed with the project. Fill all six sections before requesting approval; `ExitPlanMode` presents `design.md` only after this check passes. After approval, the Agent is reminded to update `delivery.md` with the changes, evidence, decisions, risks, open questions, and rollback notes. Set `KIMI_CODE_SPEC_QUALITY_GATE` to `fast`, `standard` (the default), `strict`, or `release` to select the evidence checklist written into that record.
