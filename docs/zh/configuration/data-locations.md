...
- **`<workspace>/specs/<name>/`**：启用 `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` 后，Plan 模式在项目内写入的文档。`spec.md` 保存需求、设计、任务清单与决策；`delivery.md` 保存交付记录；该目录不在 `KIMI_CODE_HOME` 下，因此可以随项目提交。
