...
启用 `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` 后，**`SpecTaskList`** 会维护当前规格运行的结构化任务轨迹。每项任务包含稳定 `id`、原因、风险等级、计划与实际变更的文件路径，以及验证证据。省略 `tasks` 会查询列表；提供 `tasks` 则以当前任务轨迹整体替换列表。在调用 `Write`、`Edit`、`Bash`、`Agent` 或 `AgentSwarm` 前设置 `activeTaskId`，可将这些结果自动归属到所选任务；传入 `null` 可清除选择。低风险当前任务会在用户规则、敏感文件保护和 Git 控制路径保护通过后自动批准这些受支持的变更；中风险任务沿用常规权限模式；高风险任务始终请求审批，即使处于自动模式也不例外。

**`SpecRun`** 会读取规格计划获批时保存的不可变快照和审批审计信息。执行期间调用它，可以在继续变更前核对目标、约束、验收标准、选定策略、质量门禁、计划任务、验证方式、审批来源、审批时间和所选方案。该工具不接受参数，且仅在最终获批后可用。

**`SpecDelivery`** 会根据获批时冻结的规格与设计快照、审批审计信息、结构化任务和已追踪的工具调用结果，写入项目内的 `delivery.md`。每项质量门禁证据必须提供一次成功的前台 `Bash` 调用的 `toolCallId`，使交付记录可关联到已执行的命令；后台调用不能证明验证完成。提供决策、风险、开放问题和回滚说明。仅当每个任务均已完成，且所选质量门禁要求的全部证据类别均有已验证的命令引用时，才将 `complete` 设为 `true`；否则工具不会改写记录，并会说明缺失项。
