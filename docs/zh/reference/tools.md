...
启用 `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` 后，Plan 模式会改为在项目内的 `<workspace>/specs/<plan-id>/` 创建文档：`spec.md` 初始包含目标、约束和验收标准章节，`design.md` 初始包含任务、风险和验证章节。`Write` 与 `Edit` 只允许修改这两份文档，因此它们可随项目一起审查和提交。必须填写完六个章节后才能请求审批；检查通过后，`ExitPlanMode` 才会展示 `design.md` 供用户审批。它还会将已审批文档路由到开发策略（`planning`、`agile_mvp`、`controlled_feature`、`bug_diagnosis`、`refactor`、`review`、`release` 或 `research`），并给出可解释的推荐质量门禁和必需任务类别；显式设置的 `KIMI_CODE_SPEC_QUALITY_GATE` 仍会控制实际清单。审批时，Kimi Code 会冻结规格与设计文档作为本次运行的事实来源，并记录审批来源、时间和所选方案，后续编辑不会改变交付记录。设置该变量为 `fast`、`standard`（默认）、`strict` 或 `release`，可选择写入该记录的证据清单。
...
| `SpecTaskList` | 自动放行 | 跟踪规格任务及其实现证据（实验功能） |
| `SpecRun` | 自动放行 | 查询已批准运行的目标、约束、验收标准和验证方式（实验功能） |
...
启用 `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` 后，**`SpecTaskList`** 会维护当前规格运行的结构化任务轨迹。每项任务包含稳定 `id`、原因、可选的策略 `category`、风险等级、计划与实际变更的文件路径，以及验证证据。`SpecRun` 会列出所选策略要求的类别；必须为已完成任务标记这些类别，例如故障诊断要求 `reproduction`、`root_cause` 和 `regression_test`。省略 `tasks` 会查询列表；提供它则以当前任务轨迹整体替换列表。在调用 `Write`、`Edit`、`Bash`、`Agent` 或 `AgentSwarm` 前设置 `activeTaskId`，可将这些结果自动归属到所选任务；传入 `null` 可清除选择。只有 `pending` 和 `in_progress` 任务可被设为当前任务；完成或阻塞当前任务时会自动清除选择。低风险当前任务会在用户规则、敏感文件保护和 Git 控制路径保护通过后自动批准这些受支持的变更；中风险任务沿用常规权限模式；高风险任务始终请求审批，即使处于自动模式也不例外。`SpecDelivery` 最终化运行后，任务清单和轨迹不再接受更新，并会清除当前任务；后续工作应新建规格运行。

**`SpecRun`** 会读取规格计划获批时保存的不可变快照和审批审计信息。执行期间调用它，可以在继续变更前核对目标、约束、验收标准、选定策略、必需任务类别、质量门禁、计划任务、任务进度摘要、当前任务状态、计划风险、验证方式、审批来源、审批时间、所选方案和交付最终化时间。它还会提示当前 `spec.md` 或 `design.md` 是否偏离获批快照；`SpecDelivery` 仍使用获批内容。该工具不接受参数，且仅在最终获批后可用。

**`SpecDelivery`** 会在项目内的运行目录中同时写入便于阅读的 `delivery.md` 和机器可读的 `delivery.json`。二者都使用获批时冻结的规格与设计快照、审批审计信息、结构化任务和已追踪的工具调用结果。其中 Changes 章节会将每个已记录文件关联到任务、原因、风险、策略类别和工具调用；Activity 章节会按任务记录每次命令或委托 Agent 调用及其结果。每项质量门禁证据必须提供归属于已完成规格任务的一次成功前台 `Bash` 调用的 `toolCallId`，使交付记录可关联到已验证的命令；后台调用以及未归属于已完成任务的命令都不能证明验证完成。提供决策、风险、开放问题和回滚说明。仅当每个任务均已完成、每个必需策略类别都已标记给已完成任务，且所选质量门禁要求的全部证据类别均有来自已完成任务的已验证命令引用时，才将 `complete` 设为 `true`。完成会向两份记录写入最终化时间并锁定后续更新；后续工作应新建规格运行。缺少检查项时，工具不会改写两份记录，并会说明缺失项。
