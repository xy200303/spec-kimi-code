...
**`SpecRun`** 会读取规格计划获批时保存的不可变快照和审批审计信息。执行期间调用它，可以在继续变更前核对目标、约束、验收标准、选定策略、必需任务类别、质量门禁、计划任务、当前任务状态、计划风险、验证方式、审批来源、审批时间、所选方案和交付最终化时间。它还会提示当前 `spec.md` 或 `design.md` 是否偏离获批快照；`SpecDelivery` 仍使用获批内容。该工具不接受参数，且仅在最终获批后可用。
