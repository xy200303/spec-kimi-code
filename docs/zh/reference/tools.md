...
**`SpecDelivery`** 会根据获批时冻结的规格与设计快照、审批审计信息、结构化任务和已追踪的工具调用结果，写入项目内的 `delivery.md`。其中 Changes 章节会将每个已记录文件关联到任务、原因、风险、策略类别和工具调用。每项质量门禁证据必须提供一次成功的前台 `Bash` 调用的 `toolCallId`，使交付记录可关联到已执行的命令；后台调用不能证明验证完成。提供决策、风险、开放问题和回滚说明。仅当每个任务均已完成、每个必需策略类别都已标记给已完成任务，且所选质量门禁要求的全部证据类别均有已验证的命令引用时，才将 `complete` 设为 `true`；否则工具不会改写记录，并会说明缺失项。
