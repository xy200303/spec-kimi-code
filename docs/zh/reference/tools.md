...
**`SpecDelivery`** 会根据已审批的规格与设计、结构化任务和已追踪的工具调用结果，写入项目内的 `delivery.md`。提供质量门禁证据，以及决策、风险、开放问题和回滚说明。仅当每个任务均已完成，且已提供所选质量门禁要求的全部证据类别时，才将 `complete` 设为 `true`；否则工具不会改写记录，并会说明缺失项。
