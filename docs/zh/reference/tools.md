...
启用 `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` 后，Plan 模式会在项目内的 `<workspace>/specs/<name>/` 创建文档：`spec.md` 初始包含 frontmatter 以及目标、验收标准、约束条件、技术选型、任务清单、风险与应对、关键决策、待确认问题和变更记录章节；`delivery.md` 是交付记录骨架。`Write` 与 `Edit` 在 Plan 模式下只允许修改 `spec.md`，因此它可以随项目一起审查和提交。必须完成"目标"和"验收标准"章节后，`ExitPlanMode` 才会展示 `spec.md` 供用户审批。审批后 Agent 按任务清单实施，实施过程中在 `spec.md` 中勾选任务即为进度更新；实施与验证完成后，再按模板填写 `delivery.md`。

**`EnterPlanMode`** 接受可选的 `name` 参数（语义化 kebab-case 目录名，如 `"nebula-effect"`），用于生成 `specs/<name>/`。若省略或名称非法，则使用随机名称；若目录已存在，会自动追加 `-2`、`-3` 等后缀。进入成功后返回工作流指引以及 `spec.md` / `delivery.md` 的路径。
...
启用 `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` 后，任务清单直接写在 `spec.md` 的"任务清单"章节内。Agent 在实施过程中勾选任务即为状态更新；没有独立的任务跟踪工具。
