...
启用 `KIMI_CODE_EXPERIMENTAL_SPEC_CODING=1` 后，**`SpecTaskList`** 会维护当前规格运行的结构化任务轨迹。每项任务包含稳定 `id`、原因、风险等级、计划与实际变更的文件路径，以及验证证据。省略 `tasks` 会查询列表；提供 `tasks` 则以当前任务轨迹整体替换列表。在调用 `Write`、`Edit`、`Bash`、`Agent` 或 `AgentSwarm` 前设置 `activeTaskId`，可将这些结果自动归属到所选任务；传入 `null` 可清除选择。
